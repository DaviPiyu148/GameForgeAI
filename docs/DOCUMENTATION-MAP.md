# Documentation Map

| File | Role |
|---|---|
| `AGENTS.md` | AI development constitution |
| `DESIGN.md` | Visual source of truth |
| `01-PROJECT.md` | Product definition |
| `02-PRODUCT-SPEC.md` | Product behavior |
| `03-TECH-STACK.md` | Technology decisions |
| `04-SYSTEM-ARCHITECTURE.md` | System architecture |
| `05-FRONTEND-ARCHITECTURE.md` | Current frontend |
| `06-BACKEND-ARCHITECTURE.md` | Backend architecture (implemented) |
| `07-DATA-MODEL.md` | Persisted entities |
| `08-API-CONTRACT.md` | Frontend/backend contract |
| `09-AI-GAME-GENERATION.md` | AI + DSL pipeline |
| `10-DISCOVERY-ENGINE.md` | Semantic discovery |
| `11-IMPLEMENTATION-PHASES.md` | Roadmap and gates |
| `12-TESTING-QA.md` | Quality strategy |
| `13-SECURITY.md` | Security boundaries |
| `14-DEPLOYMENT.md` | Deployment plan |
| `15-CURRENT-STATUS.md` | Living status |
| `decisions/` | Accepted ADRs |

...
from app.schemas.profile import UserProgressResponse, XPEventItem
...
def calculate_level_bounds(total_xp: int) -> Tuple[int, int, int]:
    ...
    max_defined_idx = len(LEVEL_THRESHOLDS) - 1
    level = 1
...
class ProgressionService:
    ...
    gamification level progression curves, anti-spam rate limiting, and audit events.
    """
    ...
    def get_or_create_progress(cls, db: Session, user_id: str) -> UserProgress:
        ...
        if not progress:
            # Sync with existing user level if set
            user = db.query(User).filter(User.id == user_id).first()
            user_lvl = user.level if user and user.level else 1
            # Base XP for user's level
    ...
    def check_anti_spam(
        ...
        elif event_type == "SAVE_DISCOVERY":
            # Max 1 award per steam_app_id save
            if source_ref:
                prior = (
                    db.query(XPEvent)
                    .filter(
                        XPEvent.user_id == user_id,
                        XPEvent.event_type == "SAVE_DISCOVERY",
                        XPEvent.source_reference == source_ref,
                    )
                    .first()
                )
    ...
    def grant_xp(
        ...
        # Synchronize with User model level column
        user = db.query(User).filter(User.id == user_id).first()
    ...
    def get_progress_response(cls, db: Session, user_id: str) -> UserProgressResponse:
        """Construct full UserProgressResponse with calculated level bounds and recent events."""
        progress = cls.get_or_create_progress(db, user_id)

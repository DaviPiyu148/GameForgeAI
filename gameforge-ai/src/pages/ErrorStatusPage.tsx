      ...
      <div className="bg-terminal-bg border-2 border-error glow-error overflow-hidden w-full crt-flicker delay-2">
        ...
        <div className="p-4 font-mono text-xs h-64 overflow-y-auto space-y-2 text-left">
          ...
          <div className="text-error font-bold">
            &gt; FATAL_EXCEPTION_0x8B: CORE_DUMP_INITIATED
          </div>
          
          <div className="text-on-surface-variant/80">
            [SYS_TIME: 14:02:45.093] Process terminated unexpectedly.
          </div>
          ...
          <p className="text-on-surface-variant/70 leading-relaxed pt-2 pb-1">
            Memory allocation fault in terrain mesh synthesizer at chunk index [104, -82]. Thread #4 timed out waiting for shader compilation pipeline. Quantum state collapse prevented data corruption.
          </p>
          ...
          <div className={`w-full max-w-3xl bg-surface border-2 border-error rounded-sm overflow-hidden flex flex-col shadow-[0_0_30px_rgba(255,77,77,0.15)] ${isClosingModal ? 'modal-exit' : 'modal-enter'}`} onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 bg-terminal-bg font-mono text-xs h-96 overflow-y-auto space-y-1">
              {state.compilerLogs.map((log, i) => (
                <div key={i} className={log.includes('FATAL') ? 'text-error font-bold' : log.includes('[MOD]') ? 'text-secondary-soft' : 'text-primary/80'}>
                  {log}
                </div>
              ))}
